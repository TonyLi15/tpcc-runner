...
// void run_tx(int* flag, ThreadLocalData& t_data, uint32_t worker_id,
//             TimeStampManager<Protocol>& tsm, std::vector<Request>& batch_txs,
//             std::vector<RowRegion>* my_regions, uint64_t available_regions,
//             uint64_t start, uint64_t end) {
void run_tx([[maybe_unused]] int* flag,
            [[maybe_unused]] ThreadLocalData& t_data, uint32_t worker_id,
            TimeStampManager<Protocol>& tsm, std::vector<Request>& batch_txs,
            std::vector<RowRegion>& my_regions, uint64_t regions_start,
            uint64_t regions_end, uint64_t txs_start, uint64_t txs_end) {
  ...
  for (uint64_t cur_tx = txs_start; cur_tx < txs_end; cur_tx++) {
    ...
    std::vector<Operation>& write_set =
        batch_txs[cur_tx].write_set_;  // Txθ's write set
  ...
  for (uint64_t cur_tx = txs_start; cur_tx < txs_end; cur_tx++) {
    ...
    std::vector<Operation>& ope_set =
        batch_txs[cur_tx].operations_;  // Txθ's operations
...
int main(int argc, const char* argv[]) {
  if (argc != 7) {
    printf(
        "workload_type(A,B,C,F) num_records num_threads seconds skew "
        "reps_per_txn\n");
    exit(1);
  }
  ...
  for (int i = 0; i < num_threads; i++) {
    ...
    threads.emplace_back(run_tx<Protocol>, &flag, std::ref(t_data[i]), i,
                         std::ref(tsm), std::ref(batch_txs), std::ref(regions),
                         regions_start, regions_end, txs_start, txs_end);
  }
  ...
  constexpr_for<TxProfileID::MAX>([&](auto i) {
    ...
    printf("    %-20s c:%10lu(%.2f%%)   ua:%10lu  sa:%10lu\n", Profile::name,
           stat[p].num_commits, stat[p].num_commits / (double)total.num_commits,
