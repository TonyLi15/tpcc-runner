...
uint64_t initialization_time[64];
uint64_t execution_time[64];
...
void run_tx(RendezvousBarrier &exp, RendezvousBarrier &exec_phase_start,
    ...
    // Core Assignment -> caracal: sequential
    pid_t tid = gettid();
    ...
    while (epoch <= NUM_EPOCH) {
        ...
        /*
           ...
           contend on some rows during batch-append. If this hap- pens, Caracal
    ...
    initialization_time[worker_id] = init_total;
    execution_time[worker_id] = exec_total;
}

int main(int argc, const char *argv[]) {
    if (argc != 8) {
        printf("seconds workload_type(A,B,C,F) num_records num_threads skew "
               "reps_per_txn exp_id\n");
        exit(1);
    }
    ...
    uint64_t num_records =
        static_cast<uint64_t>(std::stoi(argv[3], nullptr, 10));
    ...
    RendezvousBarrier exp(num_threads - 1), exec_phase_start(num_threads - 1),
        exec_phase_end(num_threads - 1);
    ...
    for (int i = 0; i < num_threads; i++) {
        if (i == 0) {
            threads.emplace_back(epoch_controller, std::ref(exp),
                                 std::ref(exec_phase_start),
                                 std::ref(exec_phase_end), NUM_EPOCH);
        } else {
            threads.emplace_back(
                run_tx<Protocol>, std::ref(exp), std::ref(exec_phase_start),
                std::ref(exec_phase_end), std::ref(t_data[i]), i, std::ref(tsm),
    ...
    uint64_t init_sum = 0, exec_sum = 0;
    for (int i = 0; i < 64; i++) {
        init_sum = init_sum + initialization_time[i];
        exec_sum = exec_sum + execution_time[i];
    }
    std::cout << "initialization phase[MS]: " << init_sum / 64 / CLOCKS_PER_MS
              << std::endl;
    std::cout << "execution phase[MS]: " << exec_sum / 64 / CLOCKS_PER_MS
              << std::endl;
}
