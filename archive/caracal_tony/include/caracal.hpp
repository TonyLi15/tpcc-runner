...
class Caracal {
 public:
  ...
  /*
    - void assign_txs():
      - assign transactions using round-robin method
    - void append_pending_version(): 
      - uncontended -> directly append to global version array
      - contended -> append to per-core per-row buffer
    - const Rec* Read():
      - search visibile version from global version array by binary search
    - Rec* Write():
      - fill in the pending version within actual value
  */
