...
#include "protocols/serval_rc_bbu/include/version.hpp"
...
class WriteBitmap {
  public:
    ...
    Version *append_pending_version(uint64_t core, uint64_t tx, Stat &stat) {
        lock_.lock();
        ref_cnt_++;
        Version *v = do_append_pending_version(core, tx, stat);
        assert(v);
        lock_.unlock();
        return v;
    }
    ...
    // execute read
    void decrement_ref_cnt(Stat &stat) {
        lock_.lock();
        ref_cnt_--;
        if (ref_cnt_ == 0) {
            gc_and_update_master(stat);
        }
        lock_.unlock();
    }

    // execute write
    Version *identify_write_version(uint64_t core, uint64_t tx, Stat &stat) {
        ...
        if (ref_cnt_) {
            ...
            if (is_final_state(core, tx)) {
                gc(master_, stat);
                master_ = create_pending_version(stat);
                write_v = master_;
            }
        }
        lock_.unlock();
  ...
  private:
    ...
    // read phase
    Version *do_append_pending_version(uint64_t core, uint64_t tx, Stat &stat) {
        if (core_bitmap_) {
            ...
            if (is_found) {
                ...
                if (itr == placeholders_.end()) {
                    ...
                    itr = placeholders_.insert(
                        itr, std::make_pair(visible_id,
                                            create_pending_version(stat)));
                }
                return itr->second;
            }
        }
