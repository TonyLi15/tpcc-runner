...
class WriteBitmap {
  ...
  int count_prefix_sum(uint64_t core) {
    ...
    */
    ...
    return count_bits(core_bitmap_ &
                      fill_the_left_side_until_before_the_given_position_128((int)core));
#else
    return count_bits(core_bitmap_ &
                      fill_the_left_side_until_before_the_given_position((int)core));
#endif
  }
 ...
 private:
  ...
  uint64_t get_serial_id(uint64_t core, uint64_t tx) { return core * NUM_TXS_IN_ONE_EPOCH_IN_ONE_CORE + tx; }
