...
class Plot:
    ...
    def organize_data_by_param(self, df, param):
        ...
        if "Time" in param:
            return (df[param] / CLOCKS_PER_SEC).to_numpy()
        elif "Wait" in param:
            return (df[param] / CLOCKS_PER_MS).to_numpy()
    ...
    def plot_all_param_all_protocol(self, dfs):
        for param in self.plot_params:
            ...
            fig, ax1 = plt.subplots(dpi=300)
            for protocol in self.protocols:
                df = dfs[protocol].sort_values(by=self.VARYING_TYPE)
                ax1.plot(
                    self.get_x_ticks(df, self.VARYING_TYPE),
    ...
    def histogram_of_init_and_exec_phase(self, dfs, threshold):
        ...
        for protocol in self.protocols:
            ...
            for param in ["InitializationTime","ExecutionTime"]:
              results[param].append(self.organize_data_by_param(df[df[self.VARYING_TYPE] == threshold], param)[0])
